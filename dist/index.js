#!/usr/bin/env node
import { createRequire as __nodejsCreateRequire } from 'node:module'; const require = __nodejsCreateRequire(import.meta.url);
...
// src/spec.ts
var SOURCE_FILES = ["**/*.js", "**/*.mjs", "**/*.cjs", "**/*.ts"];
var spec = {
  "id": "nodejs",
  "displayName": "Node.js",
...
var LIFECYCLE_EVENTS = /* @__PURE__ */ new Set(["abort", "close", "disconnect", "end", "error", "exit", "finish", "timeout"]);
...
function evaluate(rule, sources, allPaths) {
  ...
  if (match.kind === "event-listener-cleanup") {
    return sources.filter((file) => match.files.some((glob) => matchesGlob(file.path, glob))).flatMap((file) => findIncompleteListenerCleanup(rule, file));
  }
...
function findIncompleteListenerCleanup(rule, file) {
  ...
  for (const calls of groups.values()) {
    ...
    if (callback === void 0 || !isTeardownCallback(callback)) continue;
    const removals = findListenerCalls(callback.body, /\b(removeListener|off)\b/, callback.bodyStart);
    const removedEvents = new Set(
      removals.filter((call) => call.receiver === first.receiver && call.callback === first.callback).map((call) => call.event)
    );
...
function findNamedFunctions(source) {
  ...
  for (const definition of definitions) {
    for (const match of source.matchAll(definition)) {
      const name = match[1];
      if (name === void 0 || match.index === void 0 || functions.has(name)) continue;
      const openBrace = source.indexOf("{", match.index + match[0].length - 1);
      const closeBrace = findClosingBrace(source, openBrace);
      if (closeBrace !== void 0) functions.set(name, { name, body: source.slice(openBrace + 1, closeBrace), bodyStart: openBrace + 1 });
    }
  }
  return functions;
}
function findClosingBrace(source, openBrace) {
  if (openBrace < 0) return void 0;
  let depth = 0;
  let quote;
  let escaped = false;
...
function findListenerCalls(source, method, offset = 0) {
  ...
  for (const match of source.matchAll(expression)) {
    ...
    if (receiver !== void 0 && event !== void 0 && callback !== void 0 && match.index !== void 0) {
...
function isTeardownCallback(callback) {
  return /cleanup|teardown|dispose|release|finalize|settle/i.test(callback.name) || /\.(?:removeListener|off|destroy|close|abort|dispose|unref)\s*\(|\[\s*kUnref\s*\]\s*\(|\bclear(?:Timeout|Interval)\s*\(/.test(callback.body);
}
...
function locateEligible(file, expression, anchors) {
  ...
  for (const match of file.source.matchAll(new RegExp(expression.pattern, flags))) {
    if (match.index === void 0 || match[0] === "") continue;
    const line = file.status === "modified" && anchors !== void 0 ? eligibleExpressionAnchor(file, match.index, match[0], anchors) : eligibleRangeAnchor(file, match.index, match.index + match[0].length);
    if (line !== void 0) return { line, snippet: snippetAt(file.source, line) };
  }
  return void 0;
}
function eligibleExpressionAnchor(file, offset, matchedSource, anchors) {
  ...
  for (const anchor of anchors) {
    ...
    for (const match of matchedSource.matchAll(new RegExp(anchor.pattern, flags))) {
      ...
      if (line !== void 0 && (eligible === void 0 || line < eligible)) eligible = line;
    }
  }
  return eligible;
}
function eligibleRangesAnchor(file, ranges) {
  if (file.status !== "modified") {
    const first = ranges.reduce(
      (earliest, range) => earliest === void 0 || range.index < earliest.index ? range : earliest,
      void 0
    );
    return first === void 0 ? void 0 : lineAt(file.source, first.index);
  }
  let anchor;
  for (const range of ranges) {
    const candidate = eligibleRangeAnchor(file, range.index, range.end);
    if (candidate !== void 0 && (anchor === void 0 || candidate < anchor)) anchor = candidate;
  }
  return anchor;
}
...
// src/index.ts
function createApp() {
  const app = new Adversary({ name: "lang/nodejs", version: "0.0.13", review: { maximumFindings: 12 } });
  registerRules(app);
